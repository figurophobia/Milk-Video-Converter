...
class VideoFilterApp(tk.Tk):
    ...
    def initUI(self):
        ...
        self.title_label = tk.Label(self, text="Milk Inside a Bag of Milk Video Filter", font=font, bg="#f0f0f0")
        self.title_label.pack(pady=10)

        self.select_video_btn = tk.Button(self, text="Select Video", command=self.select_video)
        self.select_video_btn.pack(pady=5)

        self.fps_checkbox_var = tk.IntVar()
        self.fps_checkbox = tk.Checkbutton(self, text="Customize FPS", variable=self.fps_checkbox_var, command=self.toggle_fps_spinbox)
        self.fps_checkbox.pack(pady=5)

        self.fps_spinbox = tk.Spinbox(self, from_=1, to=60, state="disabled")
        self.fps_spinbox.pack(pady=5)

        self.milk_type_label = tk.Label(self, text="Select Milk Type:", font=font, bg="#f0f0f0")
        self.milk_type_label.pack(pady=5)

        self.milk_type_combo = ttk.Combobox(self, values=["Milk Type 1", "Milk Type 2"])
        self.milk_type_combo.current(0)
        self.milk_type_combo.pack(pady=5)

        self.pointillism_checkbox_var = tk.IntVar()
        self.pointillism_checkbox = tk.Checkbutton(self, text="Apply Pointillism Effect", variable=self.pointillism_checkbox_var)
        self.pointillism_checkbox.pack(pady=5)

        self.compression_checkbox_var = tk.IntVar()
        self.compression_checkbox = tk.Checkbutton(self, text="Apply Compression", variable=self.compression_checkbox_var, command=self.toggle_quality_spinbox)
        self.compression_checkbox.pack(pady=5)

        self.quality_spinbox = tk.Spinbox(self, from_=0, to=100, state="disabled")
        self.quality_spinbox.pack(pady=5)

        self.progress_bar = ttk.Progressbar(self, orient="horizontal", length=300, mode="determinate")
        self.progress_bar.pack(pady=5)

        self.progress_label = tk.Label(self, text="Progress: 0.00%", font=font, bg="#f0f0f0")
